...
# Modo que o servidor verifica os movimentos
# em um loop infinito
game_loop_mode = False
...
async def handle_request_player_move(buffer, player):
    global game_loop_mode
    print("===REQUEST PLAYER MOVE===")
    
    if(not game_loop_mode):
        # Lê a posição do player
        move_x = buffer.read_float()
        move_y = buffer.read_float()
